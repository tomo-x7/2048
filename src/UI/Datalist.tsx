...
export function Datalist({
	...
	useEffect(() => {
		...
		setdata(a);
	}, [close,localStorage.getItem("savedata")]);

	return (
		<Overlay close={close}>
			<div
				style={{ maxWidth: "calc(95vw - 40px)", maxHeight: "calc(90dvh - 40px)" }}
			...
			>
				{data.map((savedata, i) => (
					...
					>
						<Savedata close={close} savedata={data} index={i} load={load} nowdata={nowdata} forcerender={forcerender} />
					</div>
				))}
			</div>
		</Overlay>
